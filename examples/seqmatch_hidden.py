...
The produced plot suggests that loss looks like some variation of ab^(-cx)
where a, b, and c are constants and x is the number of hidden nodes.
"""
import torch
...
NUM_SEQUENCES = 3000  # this is not much data; increase this to see the
                      # effect more clearly - more hidden nodes are more
                      # hurt by the lack of data. 100k is a good amount
HPARAMS = 'fastest'  # rnns aren't the most stable so consider increasing
                     # this to fast if you have enough available CPU
TRIALS_TO_FIND_HPARAMS = 3  # increase to ~6 if you have enough available CPU
